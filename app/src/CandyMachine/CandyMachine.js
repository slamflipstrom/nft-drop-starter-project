...
const CandyMachine = ({ walletAddress }) => {
  ...
  const getCandyMachineState = async () => {
    ...
    if (data.length !== 0) {
      for (const mint of data) {
        ...
        if (!mints.find((mint) => mint === parseFloat.image)) {
  ...
  const goLiveDateTimeString = machineStats?.goLiveDateTimeString;
  const itemsRedeemed = machineStats?.itemsRedeemed;
  const itemsAvailable = machineStats?.itemsAvailable;
  ...
  return (
    machineStats && (
      <div className="machine-container">
        {renderDropTimer()}
        <p>{`Drop Date: ${goLiveDateTimeString}`}</p>
